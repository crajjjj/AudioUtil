...
| `FuzCache` | `.fuz` voice-container support: `Resolve(path)` reads the fuz through the engine's resource loader (`BSResourceNiBinaryStream`, so **BSA-packed fuz work** — any vanilla dialogue line is playable by path), parses the FUZE header (magic/version/lipSize, then LIP block, then RIFF audio), sniffs the payload form (`XWMA`→`.xwm`, `WAVE`→`.wav`) and extracts it once to `Data\Sound\AudioUtilFuzCache\<stem>_<fnv1a-hash>.<ext>`, reused across sessions (delete the folder freely; failures are session-cached). The embedded LIP data is currently skipped (future: real phoneme lipsync) |
...
- **Playable formats: wav, xwm, fuz.** Scans pick up all three (loose); `PlayFile`/file lists resolve all three from BSAs too. A `.fuz` plays via its `FuzCache`-extracted payload; captions key off the `.fuz` path itself, lipsync skips fuz (compressed xWMA payload).
