...
- The default TOML ships with **no** slots/SFX (SFW-neutral by design). A preset mod that needs custom globals ships its **own** base `AudioUtil.toml` (overwriting the neutral one via load order); pure add-ons ship additive `AudioUtil\config\*.toml` overlays. Globals (`[general]`/`[ppa]`/`[lipsync]`/`[gag]` toggles) are **base-only** — overlays that set them are ignored+warned; additive data (slots, sfx, maps, gag keywords/items) unions across all files (see the `Config` row). Whole slots are last-wins by `id` in sorted filename order.
