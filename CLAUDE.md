...
| `PapyrusAPI` | Registers all natives across `AudioUtil`, `AudioUtilPPA`, `TomlUtil`; holds actor→slot resolution (`ResolveSlotForActor`, `PickFromSlotList` with per-NPC spreading via `StableLocalID`), the `ResolveGaggedKey` gag-routing helper, the shared `PlayFromKey` helper, and `PlayFileByPath` (shared body of the PlayFile natives, also the backend of `AudioUtilAPI`) |
...
- **Playable formats: wav, xwm, fuz.** Scans pick up all three (loose); `PlayFile`/file lists resolve all three from BSAs too. A `.fuz` plays via its `FuzCache`-decoded PCM wav; captions key off the `.fuz` path itself and **fuz lines lipsync** — with **real phoneme curves** when the fuz's embedded LIP block parses (any vanilla dialogue line!), else the amplitude envelope over the decoded PCM. A loose wav gets phoneme lipsync by shipping a **same-stem `.lip`** beside it (generate offline with the CK's `LipGenerator.exe` or FaceFXWrapper — see `docs/lip-format-research.md` §4; batch tool: `tools/lipgen/make_lips.py` reads caption sidecars as the spoken text and can also pack wav+lip into `.fuz`). Raw `.xwm` files play but skip lipsync.
