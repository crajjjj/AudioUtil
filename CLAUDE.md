...
- **Playable formats: wav, xwm, fuz.** Scans pick up all three (loose); `PlayFile`/file lists resolve all three from BSAs too. A `.fuz` plays via its `FuzCache`-decoded PCM wav; captions key off the `.fuz` path itself and **fuz lines lipsync** (amplitude envelope over the decoded PCM). Raw `.xwm` files play but skip lipsync.
