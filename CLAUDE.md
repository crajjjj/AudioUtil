...
- Play: `PlayVoice(Actor, category, volume, group, channel, blockLipSync=false)`, `PlayVoiceFromSlot(slot, category, akFollow, volume, group, channel, blockLipSync=false)`, `PlaySFX(sfxName, Actor, volume, group="sfx", channel)` (resolves `sfxName` first as a category of the **sfx slot** — `[general] sfx_slot`, default `SFX0` — then the flat `[sfx]` table, via `PapyrusAPI::ResolveSfxKey`; the sfx slot gives sfx pools the full `[[slot]]` toolset incl. BSA file-lists), `PlayFile(dataRelPath, Actor, volume, group, channel)` (never lipsyncs — unchanged legacy semantics; also serves non-vocal one-shots), `PlayFileWithLipSync(...)` (same args, spoken-line variant: drives the actor's mouth like a voice call), `PlayFolder(dataRelFolder, Actor, volume, group, channel)` (no auto-lipsync; opt in via `StartLipSync(actor, handle)`). `blockLipSync=true` = play this one line without moving the mouth (per-call opt-out; a mod that owns an actor's face across many lines passes `true` on each — there is no standing per-actor block). A category listed in `[lipsync] block_categories` also never lipsyncs.
